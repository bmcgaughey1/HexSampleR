#' computeCellSize -- Compute the \code{cellsize} parameter for \code{spsample} for a sample area
#'
#' Compute the value for the \code{cellsize} parameter for the \code{spsample} function in the
#' \code{sp} package to generate a sample grid representing \code{area} square units. This is
#' a helper function for \code{generatePolygonSamplePoints} to help develop sample points
#' that represent a given sample area.
#'
#' @param area numeric: Area represented by each sample point in acres or hectares.
...
computeCellSize <- function(
  ...
  if (nchar(aunits) > 0) {
    if (nchar(pattern) > 0) {
      if (toupper(substr(aunits, 1, 1)) =="A")
        a <- area * 43560
      else
        a <- area * 10000

      hexside <- sqrt(a * 2 / (3 * sqrt(3)))
      hexcellsize <- 2 * (hexside * sqrt(3) / 2)
      squarecellsize <- sqrt(a)

      if (toupper(substr(pattern, 1, 1)) =="R")
...
#' but the number of sample points will be distributed over the parts.
#'
...
#'   generating sample points for polygons that vary in size. \code{count} should be the
#'   desired number of sample points when \code{pattern} is "random".
...
generatePolygonSamplePoints <- function(
  ...
  # check to make sure we have a value for count when pattern = "random"
  if (tolower(pattern) == "random" && count <= 0) {
    message("You must specify a value for count when creating a random sample")
    return(NULL)
  }
